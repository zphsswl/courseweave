...
![CourseWeave 跨教材主题网络与原文证据界面](./report/1027a1029699157803a689a21dd52351.png)
...
当前自动化测试覆盖：课程隔离、来源 occurrence 保留、真实页码映射、chunk 页范围、原文引用校验、无证据关系拒绝、跨教材候选范围、混合检索课程隔离、Agent 计划与质量验证、人类检查点续跑，以及公开示例数据与引用完整性。

固定评测集已扩展到 100 道教师问题（80 道可回答题，其中 15 道跨教材题；20 道课程外拒答题）。在当前 7 本教材、6,498 个 chunk 上的可复现实测结果为：

| 指标 | 结果 |
|---|---:|
| 检索召回率@8 | 80/80，100% |
| 引用准确率@3 | 206/240，85.83% |
| 跨教材覆盖率@8 | 15/15，100% |
| 无答案拒答率 | 20/20，100% |

运行 `scripts/run_teacher_benchmark.py` 可重新生成带题集哈希、逐题结果和适用边界的报告。详见 [RAG 教师问题评测报告](./docs/evaluation/teacher-benchmark-v2.md)。
...
- [100 题 RAG 教师问题评测报告](./docs/evaluation/teacher-benchmark-v2.md)
- [版本记录](./CHANGELOG.md)
