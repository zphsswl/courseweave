...
评测集放在 `backend/evals/teacher_questions.json`，已从 50 题扩展为 100 道接近教师真实备课方式的问题：80 道教材内可回答问题，其中 15 道要求跨教材覆盖，另有 20 道超出课程范围的问题。新增问题覆盖局部解剖、组织胚胎、生理、病理、病理生理、微生物、传染病和跨教材解释，并加入容易产生词面干扰的域外问题。
...
| 指标 | 当前结果 | 判定方式 |
|---|---:|---|
| 检索召回率@8 | 80/80，100% | 前 8 条覆盖问题要求的知识点 |
| 引用准确率@3 | 206/240，85.83% | 前 3 条逐条满足知识相关与有效页码 |
| 跨教材覆盖率@8 | 15/15，100% | 对比问题至少召回两本教材的相关证据 |
| 无答案拒答率 | 20/20，100% | 课程外问题不返回伪相关结果 |
| 页码可追溯率 | 6,498/6,498，100% | 每个 chunk 都能定位到有效物理页范围 |

评测脚本会导出题集版本、SHA-256、语料规模、逐题命中和失败 ID，不包含教材原文；完整结果见 `docs/evaluation/teacher-benchmark-v2.md`。同时使用自动化测试覆盖章节边界、页码偏移、语义切分、噪声过滤、课程隔离、对比召回、过期向量拒用、无答案拒答、跨教材证据范围、模型降级，以及 Agent 的计划、验证器、持久任务、检查点续跑和公开示例数据完整性。**85.83% 不是包装成“已经完美”，而是一个定义清楚、可复现、能继续回归优化的可信基线。**
...
>
> 我把固定评测集从 50 题扩展到 100 题，其中包括 80 道可回答题、15 道跨教材题和 20 道域外拒答题。在当前 7 本教材上，Recall@8 是 100%，跨教材覆盖率和拒答率都是 100%，逐条核验页码和知识相关性的 Precision@3 是 85.83%。评测脚本会记录题集哈希和逐题结果，因此这个数字可以复现，也能用于下一轮回归。它更准确地说是一个垂直领域的 Agentic RAG，而不是完全自主 Agent；它的核心价值是把多本教材变成可验证、可比较、可复用的课程知识资产。
