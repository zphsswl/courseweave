...
同时使用 88 项自动化测试覆盖章节边界、页码偏移、语义切分、噪声过滤、课程隔离、对比召回、过期向量拒用、无答案拒答、跨教材证据范围、模型降级，以及 Agent 的计划、验证器、持久任务和检查点续跑。这里最重要的产品判断是：**77.5% 不是包装成“已经完美”，而是一个可以继续优化、可以回归比较的可信基线。**
...
- 部署：FastAPI 同源托管前端，支持 Docker / Render 与公开只读作品模式。
...
它仍不是“完全自主的通用 Agent”：工具集和质量规则受领域限定，不会自由执行任意操作，最终教学发布权仍属于教师。这正是产品边界，不是能力缺失。面试时推荐表述为：
...
项目展示的不只是调用大模型，而是完整的产品闭环：真实用户问题、数据管线、质量指标、证据可信度、失败降级、人工审核、任务稳定性、交互设计和部署边界。它能证明产品判断是“让 AI 结果可用、可验证、可运营”，而不是只追求一个看起来聪明的 Demo。
