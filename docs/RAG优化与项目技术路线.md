...
- 对教师问句中的“由什么组成、受哪些因素调节、各阶段”等提问框架先做语义清洗，并统一中文术语内的空格、连字符差异，避免把问法误当成必须出现在教材里的知识主题。
...
评测集已升级为 `medical-teacher-v3`，共 420 题。它保留 100 道人工核心题，并通过可复现脚本从真实章节元数据和知识节点生成 274 道章节覆盖题（137 章、每章 2 题），再加入 26 道跨教材扩展题和 20 道域外拒答扩展题。最终包含 380 道教材内可回答题（其中 41 道跨教材题）和 40 道拒答题，覆盖定义、结构、机制、过程、分类、功能、诊断、防治、临床、比较和拒答等题型。生成文件只保存问题、知识点 rubric、章节标识与页码范围，不保存教材段落原文。

判分规则将“找全”和“引对”拆开：单概念必须命中该概念；双概念必须两侧都命中；多项枚举至少覆盖 60%；同一概念的中文、英文或符号别名只计一个概念。Recall@8 负责衡量结果集是否找全，Precision@3 则逐条检查引用是否包含预期知识并具有合法物理页码，避免一个遗漏项把其余正确引用重复判错。

在当前 7 本教材数据上重新执行得到：

| 指标 | 当前结果 | 判定方式 |
|---|---:|---|
| 检索召回率@8 | 380/380，100% | 前 8 条覆盖问题要求的知识点 |
| 引用准确率@3 | 985/1,134，86.86% | 前 3 条逐条满足知识相关、目标章节与有效页码 |
| 跨教材覆盖率@8 | 41/41，100% | 对比问题至少召回两本教材的相关证据 |
| 无答案拒答率 | 40/40，100% | 课程外问题不返回伪相关结果 |
| 章节题命中率@8 | 274/274，100% | 每道章节题命中指定教材、章节与知识点 |
| 章节覆盖率@8 | 137/137，100% | 每个一级章节至少有一道问题成功命中 |
| 页码可追溯率 | 6,498/6,498，100% | 每个 chunk 都能定位到有效物理页范围 |

`scripts/build_chapter_benchmark.py` 负责确定性重建章节题，`scripts/run_teacher_benchmark.py` 会导出题集版本、SHA-256、语料规模、逐题命中、子集与题型分层，不包含教材原文；完整结果见 `docs/evaluation/teacher-benchmark-v3.md`。同时使用自动化测试覆盖章节边界、页码偏移、语义切分、噪声过滤、课程隔离、对比召回、过期向量拒用、无答案拒答、跨教材证据范围、模型降级，以及 Agent 的计划、验证器、持久任务、检查点续跑和公开示例数据完整性。**86.86% 的严格 Precision@3 仍然保留相邻背景段造成的失分，没有把章节覆盖题包装成“已经完美”；它是一个定义清楚、可复现、能继续回归优化的基线。**
...
>
> 我把固定评测集扩展到 420 题：保留 100 道人工核心题，并为 137 个章节各生成 2 道可追溯题，再加入跨教材和域外拒答扩展题。在当前 7 本教材上，Recall@8、41 道跨教材覆盖、40 道拒答和 137 章覆盖均为 100%，逐条核验知识相关性、目标章节和页码的 Precision@3 为 86.86%。题库生成和评测脚本都会记录题集哈希、语料哈希与逐题结果，因此数字可复现，也能用于下一轮回归。它更准确地说是一个垂直领域的 Agentic RAG，而不是完全自主 Agent；它的核心价值是把多本教材变成可验证、可比较、可复用的课程知识资产。
