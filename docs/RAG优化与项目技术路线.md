...
>
> 我把固定评测集扩展到 420 题：保留 100 道人工核心题，并为 137 个章节各生成 2 道可追溯题，再加入跨教材和域外拒答扩展题。在当前 7 本教材上，Recall@8、41 道跨教材覆盖、40 道拒答和 137 章覆盖均为 100%，逐条核验知识相关性、目标章节和页码的 Precision@3 为 86.86%。题库生成和评测脚本都会记录题集哈希与逐题结果，因此数字可复现，也能用于下一轮回归。它更准确地说是一个垂直领域的 Agentic RAG，而不是完全自主 Agent；它的核心价值是把多本教材变成可验证、可比较、可复用的课程知识资产。
